...
# Dev-only data: ensure Tanishq's admin account exists
if Rails.env.development?
  tanishq_dev = User.find_or_initialize_by(email: "tanishqgoyal590@gmail.com")
  if tanishq_dev.new_record?
    tanishq_dev.assign_attributes(
      type: nil,
      display_name: "Tanishq Goyal",
      first_name: "Tanishq",
      last_name: "Goyal",
      avatar: "https://api.dicebear.com/9.x/identicon/svg?seed=tanishq",
    ...
    tanishq_dev.save!
    puts "Created dev admin user: Tanishq Goyal (##{tanishq_dev.id})"
  else
    tanishq_dev.update!(roles: %w[admin]) unless tanishq_dev.roles.include?("admin")
    puts "Dev admin user Tanishq Goyal (##{tanishq_dev.id}) — roles: #{tanishq_dev.roles.join(', ')}"
  end
end
...
# Dev-only: time audit reviewer sample data
if Rails.env.development?
  seed_start = Date.new(2026, 1, 5)

  ta_reviewer_data = [
    { display_name: "Priya Nair",    email: "seed_ta_priya@example.com",  roles: %w[time_auditor] },
    { display_name: "Omar Khalil",   email: "seed_ta_omar@example.com",   roles: %w[time_auditor] }
  ]

  ta_reviewer_data.each do |attrs|
    next if User.exists?(email: attrs[:email])
    User.insert_all!([ {
      display_name: attrs[:display_name],
      email:        attrs[:email],
  ...
  ta_reviewer_data.each { |a| User.where(email: a[:email]).update_all(roles: a[:roles]) }

  priya = User.find_by!(email: "seed_ta_priya@example.com")
  omar  = User.find_by!(email: "seed_ta_omar@example.com")
  alice = User.find_by!(email: "seed_alice@example.com")
  bob   = User.find_by!(email: "seed_bob@example.com")

  # Reviews per week: [week_offset, count, avg_hours]
  # avg_hours is the mean approved hours per review that week
  ta_distributions = {
    priya.id => [
      [ 0, 9, 22 ], [ 1, 11, 25 ], [ 2, 10, 20 ], [ 3, 8, 30 ], [ 4, 13, 18 ],
      [ 5, 12, 24 ], [ 6, 7, 28 ], [ 8, 11, 22 ], [ 9, 14, 19 ], [ 10, 10, 26 ],
      [ 11, 12, 21 ], [ 12, 9, 23 ], [ 13, 11, 20 ], [ 14, 13, 18 ], [ 15, 10, 25 ],
      [ 16, 12, 22 ], [ 17, 8, 30 ], [ 18, 11, 24 ], [ 19, 10, 20 ], [ 20, 3, 22 ]
    ],
    omar.id => [
      [ 4, 6, 20 ], [ 5, 8, 22 ], [ 6, 7, 25 ], [ 8, 9, 18 ], [ 9, 11, 20 ],
      [ 10, 8, 24 ], [ 11, 10, 22 ], [ 12, 7, 28 ], [ 13, 9, 20 ], [ 14, 11, 18 ],
      [ 15, 8, 25 ], [ 16, 10, 22 ], [ 17, 7, 26 ], [ 18, 9, 20 ], [ 19, 8, 22 ], [ 20, 2, 20 ]
    ],
    alice.id => [
      [ 0, 5, 24 ], [ 1, 6, 20 ], [ 2, 4, 28 ], [ 3, 5, 22 ], [ 4, 7, 18 ],
      [ 5, 6, 25 ], [ 8, 5, 22 ], [ 9, 7, 20 ], [ 10, 4, 26 ], [ 11, 6, 21 ],
      [ 12, 5, 23 ], [ 13, 7, 19 ], [ 14, 6, 24 ], [ 15, 5, 22 ], [ 16, 6, 20 ],
      [ 17, 4, 28 ], [ 18, 6, 22 ], [ 19, 5, 20 ], [ 20, 2, 22 ]
    ],
    bob.id => [
      [ 0, 7, 20 ], [ 1, 8, 22 ], [ 2, 6, 25 ], [ 3, 7, 20 ], [ 4, 9, 18 ],
      [ 5, 8, 22 ], [ 6, 7, 24 ], [ 7, 6, 20 ], [ 8, 8, 22 ], [ 9, 7, 25 ],
      [ 10, 6, 20 ], [ 11, 8, 22 ], [ 12, 7, 24 ], [ 13, 9, 18 ], [ 14, 6, 22 ],
      [ 15, 8, 20 ], [ 16, 7, 25 ], [ 17, 6, 22 ], [ 18, 8, 20 ], [ 19, 7, 22 ], [ 20, 2, 20 ]
    ]
  }

  # Wipe existing TA seed data (identified by description "seed-ta")
  ta_student = User.find_by(email: "seed_student@example.com")
  if ta_student
    seed_ta_project_ids = Project.where(user: ta_student, description: "seed-ta").pluck(:id)
    if seed_ta_project_ids.any?
      ta_ship_ids = Ship.where(project_id: seed_ta_project_ids).pluck(:id)
      TimeAuditReview.where(ship_id: ta_ship_ids).delete_all
      Ship.where(id: ta_ship_ids).delete_all
      Project.where(id: seed_ta_project_ids).delete_all
    end
  end

  rng = Random.new(99)

  ta_distributions.each do |reviewer_id, weeks|
    weeks.each do |week_offset, count, avg_hours|
      ts = (seed_start + (week_offset * 7).days + 2.days).to_time

      project_rows = count.times.map { |i|
        { name: "TA Seed #{reviewer_id}-#{week_offset}-#{i}", description: "seed-ta",
          user_id: ta_student&.id || User.first.id,
          created_at: ts, updated_at: ts }
      }
      project_ids = Project.insert_all!(project_rows, returning: :id).map { |r| r["id"] }

      ship_rows = project_ids.map { |pid|
        { project_id: pid, ship_type: 0, status: 1, justification: "seed-ta",
          created_at: ts, updated_at: ts }
      }
      ship_ids = Ship.insert_all!(ship_rows, returning: :id).map { |r| r["id"] }

      ta_rows = ship_ids.map { |sid|
        # Vary hours ±40% around avg so the bars have natural variation
        hours = (avg_hours * (0.6 + rng.rand * 0.8)).round
        { ship_id: sid, reviewer_id: reviewer_id,
          status: TimeAuditReview.statuses[:approved],
          approved_public_seconds: hours * 3600,
          created_at: ts, updated_at: ts }
      }
      TimeAuditReview.insert_all!(ta_rows)
    end
  end

  total_ta = TimeAuditReview.where(reviewer_id: [ priya.id, omar.id ]).count
  puts "Seeded #{total_ta} time audit reviews across #{ta_reviewer_data.map { |r| r[:display_name] }.join(', ')}"
end

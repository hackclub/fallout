...
- `DebtSnapshotBuilder` (`app/services/debt_snapshot_builder.rb`) — the admin **debt console** (`Admin::DebtController`) decides who *entered* debt from a frozen `DebtSnapshot` of each ticket-holder's approved hours **as of a fixed cutoff** (`DebtSnapshot::CUTOFF`, July 1 2026 Pacific): `entered_debt = snapshot_approved_hours < 60`, so the roster of who owes is fixed by the deadline. Debtors *clear* by reaching 60 **live** approved hours (`in_debt = entered_debt && live < 60`) and stay on the page as resolved cases; every displayed figure (approved hours, per-project breakdown, progress/remaining) is live via `batch_user_approved_seconds`. The builder mirrors the `batch_user_approved_seconds` math but sources ship status/`approved_public_seconds`, journal kept/ship_id/author state, and journal-collaborator membership from **PaperTrail** (`version_at(cutoff)`); clip **durations stay live** (not versioned) and the attributable-project *set* is taken from current `projects_attributable_to_self_ids` (both documented pragmatic hybrids). The cutoff is immutable past, so it's built **once** offline via `bin/rails debt:snapshot` and read in O(1). A holder with no snapshot row reads as 0h at the cutoff ⇒ entered debt; if no snapshot exists at all the page refuses to compute (shows a "run debt:snapshot" banner via `snapshot_built`). PaperTrail reification requires the YAML allowlist in `config/application.rb` (`yaml_column_permitted_classes`). If you change any approved-attribution rule above, the builder must change in lockstep.
...
- `ship?`: verified, non-trial owner; blocked while a `pending` or `awaiting_identity` ship exists; first-time submissions blocked by `:disable_new_submissions`; also blocked by the two `:final_reviews` wind-down rules (`final_review_used?`, `resubmit_deadline_passed?`).
- `reship?`: same owner gate, requires a `pending` ship, and blocked by `final_review_used?` — swapping an in-flight ship stays allowed right up until a reviewer verdict lands.
- `ship_block_reason`: `:submissions_closed` / `:final_review_used` / `:deadline_passed` / `nil` — serialized as `can.ship_block_reason` so the Submit button stays clickable and raises the matching popup (`SHIP_BLOCK_MESSAGES` in `@/lib/notifications`). Returns `nil` for anyone who has no business submitting, so the button stays hidden for them.

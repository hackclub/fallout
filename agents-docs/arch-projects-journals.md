...
- `DebtSnapshotBuilder` (`app/services/debt_snapshot_builder.rb`) — the admin **debt console** (`Admin::DebtController`) does **not** judge debt against live `batch_user_approved_seconds`; it reads a frozen `DebtSnapshot` of each ticket-holder's approved hours **as of a fixed cutoff** (`DebtSnapshot::CUTOFF`, July 1 2026 Pacific), so work approved after the cutoff can't retroactively clear debt. The builder mirrors the `batch_user_approved_seconds` math but sources ship status/`approved_public_seconds`, journal kept/ship_id/author state, and journal-collaborator membership from **PaperTrail** (`version_at(cutoff)`); clip **durations stay live** (not versioned) and the attributable-project *set* is taken from current `projects_attributable_to_self_ids` (both documented pragmatic hybrids). The cutoff is immutable past, so it's built **once** offline via `bin/rails debt:snapshot` and the page reads it in O(1). `in_debt = snapshot_approved_hours < 60`; a holder with no snapshot row reads as 0h ⇒ in debt, and the page refuses to compute (shows a "run debt:snapshot" banner via `snapshot_built`) if no snapshot exists at all. If you change any approved-attribution rule above, the builder must change in lockstep.

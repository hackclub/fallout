...
- **Reviewer time-on-task** (e.g. for volunteer hours): nothing stores when a review was started — claims are written with `update_all`/`update_columns`, so PaperTrail never sees them. `rake reviewers:hours REVIEWER=<id> [QUEUE=rc|dr|br|ta CAP=30 GAP=5 SINCE= DETAIL=1]` (`lib/tasks/reviewer_hours.rake`, read-only) reconstructs it from the Ahoy `$view` events that `ApplicationController#track_page_view` writes for every request, including the 2-minute heartbeats. Each terminal review is anchored at `completed_at` and walked backwards to the `show` that opened it, so pages merely opened and skipped never count; sessions are capped at `CAP` minutes (idle tabs) and clamped to the previous review's `completed_at` (no double counting). Reviews with no event trail are reported separately and imputed at the median. `/admin/reviewers/:id` still shows counts only — its `ta_hours` is student hours *approved*, not reviewer time.

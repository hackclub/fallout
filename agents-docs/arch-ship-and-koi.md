...
**"Users that can get a ticket" filter (all four queues)**: the `ticket=eligible|all` param (persisted in session like `sort`, parsed by `BaseController#parse_ticket_filter`) restricts `pending_reviews` to ships whose **owner** currently qualifies for a summit ticket. Applied right after the pending list loads, before precompute, via `filter_ticket_eligible` — collects distinct owners (already loaded through the index `includes`) and keeps reviews whose owner's `User#meets_ticket_hours?` is true. Eligibility is `approved_time_logged_seconds` (rounded to 0.1h) ≥ `User#ticket_hours_threshold` (`ticket_hours_override || User::TICKET_HOURS_THRESHOLD`, default 60) — the same source of truth the ticket-claim flow uses. The frontend toggle is the shared `TicketFilterButton`; the eager `ticket_eligible` boolean prop reflects current state. The filter does **not** flow into `next`/`next_eligible` (Start Reviewing ignores it).

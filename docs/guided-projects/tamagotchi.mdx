...
I then connect GND to GND, 5V to +5V, +3V3 to 3V3, and BAT to +BATT as shown below:

![Power connections](https://files.catbox.moe/kgmyi6.png)

## What is 5V? What is GND? What is 3V3?
...
>
> #### 5V vs 3V3, why two rails?
>
> The XIAO exposes two voltage rails: **5V** and **3V3**.
>
> - **5V** is powered by USB-C. When running on battery, the LiPo only outputs ~3.7V, not enough to supply this pin, so it gets no power.
> - **3V3** is a stable 3.3V generated on-board, available whether you're on USB or battery.
>
> Most components, the OLED, sensors, logic pins, are rated for **3.3V**, so connect them to 3V3. Too much voltage means too much current, which damages components.

    ...
    <header className="flex justify-between relative items-start">
      <div ref={containerRef} className="flex items-start">
        <img
          src={avatar}
          alt={displayName}
          className="top-0 left-0 rounded-full aspect-square size-10 xs:size-16 bg-dark-brown border-2 border-dark-brown w-fit z-12"
        />
        <div className="flex flex-col -ml-5 xs:-ml-8 hidden xs:inline">
          <button
            type="button"
            aria-expanded={isOpen}
            aria-haspopup="menu"
            onClick={() => setIsOpen(!isOpen)}
            className={`h-10 xs:h-16 bg-light-brown pl-10 border-2 border-dark-brown text-dark-brown text-xl flex items-center rounded-r-full z-20 cursor-pointer transition-all duration-200 ${isOpen ? 'pr-5 min-w-40' : 'pr-3 min-w-0 xs:pr-5 xs:min-w-40'}`}
          >
            <span className="-mt-0.5">{displayName}</span>
            <span className={`-mt-0.5 xs:hidden transition-all duration-200 overflow-hidden whitespace-nowrap ${isOpen ? 'max-w-40 opacity-100' : 'max-w-0 opacity-0'}`}>{displayName}</span>
          </button>
